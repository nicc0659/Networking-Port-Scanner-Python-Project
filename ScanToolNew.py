...
def packet_analyzer(packet, progress):
    output_text.insert(tk.END, f"{packet}\n")
    output_text.update_idletasks()
    progress.stop()

def start_packet_analyzer(progress):
    ...
    capture.apply_on_packets(lambda packet: packet_analyzer(packet, progress), timeout=10)
...
scan_button = ttk.Button(tab1, text="Scan", command=lambda: threading.Thread(target=port_scanner, args=(progress_bar(tab1),)).start())
...
analyze_button = ttk.Button(tab3, text="Analyze", command=lambda: threading.Thread(target=start_packet_analyzer, args=(progress_bar(tab3),)).start())
